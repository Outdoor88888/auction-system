...
public class SellServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        out.println("開始日時: ");
        printDateSelect(out, "b", curYear, request);
        out.println("<br>");

        // 日時入力 (終了)
        out.println("終了日時: ");
        printDateSelect(out, "e", curYear, request);
        out.println("<br>");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(isEmpty(name)) returnError(request, response, "商品名は必須です", "name");
            ...
            try {
                ...
                String bStr = String.format("%04d-%02d-%02d %02d:%02d:00",
                        Integer.parseInt(bYear), Integer.parseInt(bMonth), Integer.parseInt(bDay), Integer.parseInt(bHour), Integer.parseInt(bMin));
                Timestamp beginAt = Timestamp.valueOf(bStr);

                String eStr = String.format("%04d-%02d-%02d %02d:%02d:00",
                        Integer.parseInt(eYear), Integer.parseInt(eMonth), Integer.parseInt(eDay), Integer.parseInt(eHour), Integer.parseInt(eMin));
                ...
                try (Connection conn = DriverManager.getConnection("jdbc:postgresql://" + _hostname + ":5432/" + _dbname, _username, _password)) {
                    ...
                    ps.setInt(4, uid);
                    ps.setTimestamp(5, beginAt);
                    ps.setTimestamp(6, endAt);
                    ps.executeUpdate();
    ...
    private void returnError(HttpServletRequest req, HttpServletResponse res, String msg, String field) throws ServletException, IOException {
        ...
        req.setAttribute("problemField", field);
        // 入力値保持
        ...
        // 日付保持
        String[] prefixes = {"b", "e"};
        String[] suffixes = {"_year", "_month", "_day", "_hour", "_min"};
        for(String p : prefixes) {
            for(String s : suffixes) req.setAttribute(p+s, req.getParameter(p+s));
        }
        doGet(req, res);
    }
}